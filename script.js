const fortunas = [
    ...
    Array.from({length: 292}, (_, i) => `Fortuna número ${i + 6}`)
...
const MILISEGUNDOS_24H = 24 * 60 * 60 * 1000;
const ultimaFortuna = localStorage.getItem("ultimaFortuna");
const fortunaGuardada = localStorage.getItem("fortunaMostrada");

// Mostrar mensaje por defecto al cargar
if (ultimaFortuna) {
const tiempoTranscurrido = Date.now() - Number(ultimaFortuna);
if (tiempoTranscurrido < MILISEGUNDOS_24H) {
    btnFortuna.disabled = true;
    resultado.innerHTML = `
    <p>Haz recibido tu fortuna diaria</p>
    <p style="margin-top: 10px; font-style: italic;">"${fortunaGuardada}"</p>
    `;
} else {
    resultado.textContent = "Presiona el botón para ver tu fortuna.";
}
} else {
resultado.textContent = "Presiona el botón para ver tu fortuna.";
}
...
if (!activo || btnFortuna.disabled) return;

  const mensaje = mensajesMagicos[Math.floor(Math.random() * mensajesMagicos.length)];
resultado.innerHTML = `
    <p>${mensaje}</p>
    <img src="https://media3.giphy.com/media/Gmb9dijBtUIuE4MW6R/giphy.gif" alt="Cargando fortuna" style="max-width: 150px; margin-top: 10px;">
`;
resultado.classList.add("visible");
...
timeoutID = setTimeout(() => {
    ...
    resultado.innerHTML = `
    <p>Haz recibido tu fortuna diaria</p>
    <p style="margin-top: 10px; font-style: italic;">"${fortuna}"</p>
    `;

    // Guardar hora y fortuna
    localStorage.setItem("ultimaFortuna", Date.now().toString());
    localStorage.setItem("fortunaMostrada", fortuna);
    btnFortuna.disabled = true;
}, 5000);
});
...
space.style.display = activo ? "inline-block" : "none";

clearTimeout(timeoutID);
resultado.classList.remove("visible");

if (!activo) {
    resultado.innerHTML = ""; // Oculta mensaje al apagar
} else {
    const ultima = localStorage.getItem("ultimaFortuna");
    const fortunaAnterior = localStorage.getItem("fortunaMostrada");
    const tiempo = Date.now() - Number(ultima);

    if (ultima && tiempo < MILISEGUNDOS_24H) {
    resultado.innerHTML = `
        <p>Haz recibido tu fortuna diaria</p>
        <p style="margin-top: 10px; font-style: italic;">"${fortunaAnterior}"</p>
    `;
    } else {
    resultado.textContent = "Presiona el botón para ver tu fortuna.";
    }
